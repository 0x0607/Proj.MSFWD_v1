...
class syslog
{
     ...
     * @param string $wid 網站編號
     * @param string $operator 操作者編號
     * @param string $logInformation 記錄檔資訊  
     * [   
     * "ip_address" => String
     * "status" => String
     * "action" => String
     ...
     * ]
     ************************************************/
    public function addSystemLog($id, $wid, $operator, $logInformation = []): bool
    {
        $params = [
            $id,                                                                                // 記錄檔編號
            $wid,                                                                               // 網站編號
            $operator,                                                                          // 操作者
            isset($logInformation['ip_address']) ? $logInformation['ip_address'] : "0.0.0.0",   // 網際網路位址
            isset($logInformation['status']) ? $logInformation['status'] : "1|OK",              // 狀態
            isset($logInformation['action']) ? $logInformation['action'] : "UNKNOWN",           // 操作
            isset($logInformation['reason']) ? $logInformation['reason'] : "No reason.",          // 原因
            hash("sha256", $id . $wid . $operator)                                              // 雜湊
        ];
        $sql = "INSERT INTO `{$this->systemLog}` VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
